...
import { ClientSearchComponent, default as TidalSearch } from '@/components/tidalSearch.tsx';
...
export async function getSearchResults(query: string): Promise<TidalTrack[] | undefined> {
    if (!searchClient) {
...
export async function InitTidal({...props}: TidalProps) {
...
export default async function SearchComponentInternal({...props}: {clientId: string, clientSecret: string}) {
    return (
